...
define('LOG_URL', SITE_URL.'log.txt');
define('LOG_PATH', SITE_PATH.DS.'log.txt');
define('DB_URL', SITE_URL.'users.json');
define('DB_PATH', SITE_PATH.DS.'users.json');

/* пришла наша форма */
if ($_POST['id']=='ok')
	{
	/* чтобы заметить спинер на странице */
	sleep(1);
	/* на всякий пожарный */
	$p = $_POST;
	/* будем считать, что это база */
	$db = json_decode(file_get_contents(DB_PATH),true);
	/* определим массив на выдачу чтобы избежать null-ов */
	$json = array(
		'ans'=>'',
		'class_alert'=>'',
		'view_text'=>array()
		);
	/* валидируем email (лучше положится на стандарты) */
	...
	/* второй пароль уже не нужен, мы все поняли */
	unset($p['passwordr']);
	/* маст хев */
	$p['password'] = md5($p['password']);
	/* лучше представить email как PRIMARY KEY чем перебирать весь массив (для выбранного варианта базы) */
	if ($db[$p['email']])
		{
		$msg = 'email duble';
		$json['view_text'][] = '<p>- This email has already been registered.</p>';
		}
	/* если что-то пришло не то - выдать ахтунг */
	if (count($json['view_text'])>=1)
		{
		$json['ans'] = 'warning';
		$json['class_alert'] = 'alert-warning';
		array_unshift($json['view_text'],'<h5>WARNING!</h5>');
		}
	/* теоретически все норм: сохранить в базу и выдать окей. тут подробнее */
	else
		{
		$msg = 'user add';
		/* добавил тайминг для "а если мы ..." */
		$p['time_change'] = $p['time_add'] = $date_time;
		/* присвоим id - поскольку мы его не используем для идентификации, можно просто порядковый номер */
		$p['id'] = number_format(count($db)+1,0,'','');
		/* наш PRIMARY KEY */
		$db[$p['email']] = $p;
		/* сохраняем без проверки разрешений на запись, подразумеваем, что можно */
		file_put_contents(DB_PATH,json_encode($db,JSON_UNESCAPED_UNICODE));
		/* ну и наш ответ Чемберлену */
		$json['ans'] = 'useradd';
		$json['class_alert'] = 'alert-success';
	...
	/* логируем */
	file_put_contents(LOG_PATH,$time1."\t".$date_time."\t".$msg."\t".json_encode($p,JSON_UNESCAPED_UNICODE)."\n",FILE_APPEND);
	/* выводим результат тяжкого труда */
	...
	{
	/* всех шаромыжников, что лезут мимо кассы - посылаем на главную страницу */
	header('Location: '.SITE_URL);
	die;
	}
/* HTML можно было проинклюдить, но так быстрее будет работать */
?>
<!doctype html>
<html lang="ru">
	<head>
	...
	<body class="d-flex flex-column h-100">
					...
					<div class="collapse navbar-collapse" id="navbarCollapse">
						...
						<div class="d-flex">
							<div class="btn-group" role="group" aria-label="">
								<button class="btn btn-primary" type="button" data-bs-toggle="modal" data-bs-target="#modalform">Sign in</button>
							</div>
						</div>
					</div>
				</div>
		...
		<main class="flex-shrink-0 mt-3">
			<div class="container col-lg-3 mx-auto">
				...
				<div class="mb-4 text-center"><button class="btn btn-primary" type="button" data-bs-toggle="modal" data-bs-target="#modalform">Sign in</button></div>
		...
		<div class="modal fade" id="modalform" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					<form id="form" class="needs-validation" target="#" method="POST" novalidate>
						...
						<div class="modal-body">
							...
							<div class="input-group mb-3">
								...
								<button type="button" class="input-group-text btn btn-success" id="password_random">Gen</button>
							...
							<div class="form-check">
								...
								<label class="form-check-label" for="email_required">Check email of PHP</label>
							</div>
						</div>
					...
					<div class="align-self-center rounded fade" id="roller">
						<div id="roller_box" class="align-middle text-center">
							<div class="lds-roller"><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div>
						</div>
					</div>
					<div class="align-self-center rounded fade" id="message">
						<div id="message_box" class="align-middle text-center alert" role="alert"></div>
					</div>
				</div>
			</div>
		</div>
